...
mat matrix_mul_orto(mat x, mat y)
{
	if (x->n != y->m) return 0;
	mat r = matrix_new(x->m, y->n);
	for (int i = 0; i < x->m; i++)
...
double FrobeniusDist(mat a, mat b) {
	double norm = 0;

	for (int i = 0; i < a->n; i++)
		for (int j = 0; j < a->m; j++) {
			double v = a->v[i][j] - b->v[i][j];
			norm += v*v;
		}
...
//int n, double a[][n]
void SchurDecomposition(int n, double _A[][n], double _U[][n], double _D[][n]) {
	...
	do {
		...
		matrix_transpose(U);
		B = matrix_mul(B, U);
		printf("U_k * A_k * U_k[t] = \n"); PrintMatr(B);
		matrix_transpose(U);
		iters++;
	} while (FrobeniusDist(A, X) > 1e-5);

	printf("U = \n"); PrintMatr(U);
	printf("U * Ut = \n"); PrintMatr(matrix_mul_orto(U, U));
	for (int i = 0; i < n; i++)
		for (int j = 0; j < n; j++) {
			_D[i][j] = A->v[i][j];
			_U[i][j] = U->v[j][i];
